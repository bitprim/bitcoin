...
void UDPRelayBlock(const CBlock& block) {
...

    {
        ...
        if (fBench) {
            ...
            LogPrintf("UDP: Built all FEC chunks for block %s in %lf %lf %lf %lf %lf %lf ms\n", hashBlock.ToString(), to_millis_double(initd - start), to_millis_double(coded - initd), to_millis_double(feced - coded), to_millis_double(header_sent - feced), to_millis_double(block_coded - header_sent), to_millis_double(all_sent - block_coded));
            if (!inUDPProcess)
    ...
    while (true) {
        ...
        if (process_block.second->block_data.IsBlockAvailable()) {
            ...
            if (status != READ_STATUS_OK) {
                ...
                std::lock_guard<std::recursive_mutex> udpNodesLock(cs_mapUDPNodes);
                setBlocksReceived.insert(process_block.first.first);
            ...
            for (size_t i = 0; !fDone; i++) {
                ...
                if (res != READ_STATUS_OK) {
                    ...
                    if (process_block.second->decoder.DecodeReady())
